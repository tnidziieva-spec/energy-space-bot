...
BOT_TOKEN = "ВСТАВИТИ_ТОКЕН_ВІД_BOTFATHER"  # ← сюди токен
CHANNEL_URL = "https://t.me/energyspace"     # ← сюди посилання на свій канал
...
def items_kb(cat_key):
    ...
    for i in range(0, len(items), 2):
        ...
        for key in pair:
            ...
            row.append(KeyboardButton(text=f"{it['emoji']} {it['name']} ({prices}₴)"))
        rows.append(row)
...
def sizes_kb(item_key):
    it = MENU[item_key]
    sizes = it["sizes"] if isinstance(it["sizes"], list) else list(it["sizes"])
    row = [KeyboardButton(text=f"{'M' if s=='M' else 'L'} — {it['prices'][s]}₴") for s in sizes]
...
def extras_inline(selected):
    ...
    for key, val in EXTRAS.items():
        ...
        buttons.append([InlineKeyboardButton(text=f"{check}{val['name']} +{val['price']}₴", callback_data=f"extra_{key}")])
...
def order_summary(order):
    ...
    lines = [f"{item['emoji']} <b>{item['name']}</b> ({size_label})"]
    milk_key = order.get("milk")
    if milk_key and milk_key in MILK_OPTIONS:
        lines.append(f"🥛 {MILK_OPTIONS[milk_key]['name']}")
    sugar = order.get("sugar", 0)
    lines.append(f"🍬 {'Без цукру' if sugar == 0 else str(sugar) + ' ч.л. цукру'}")
    ...
    lines.append(f"\n💳 <b>{calc_price(order)}₴</b>")
    return "\n".join(lines)

def find_item_by_text(text):
    for key, item in MENU.items():
        if text.startswith(f"{item['emoji']} {item['name']}"):
            return key
    return None
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"Привіт! 👋 Це бот <b>Energy Space</b>.\n\n"
        f"Поки ти йдеш у ліфті — твоя кава вже готується ☕\n\n"
        f"Тут ти можеш:\n"
        f"• <b>Замовити каву</b> за 2–3 кліки\n"
        f"• Дізнатись про <b>найближчі події</b> у нашому просторі\n"
        f"• Підписатись на наш <b>Telegram-канал</b>"
        f"{event_block}",
        parse_mode="HTML",
...
async def go_home(message: Message, state: FSMContext):
    await state.clear()
    uid = message.from_user.id
    await message.answer("Головне меню ☕", reply_markup=main_menu_kb(uid in user_history))
...
async def show_events(message: Message):
    ...
    await message.answer("Підписуйся, щоб не пропустити нові події:", reply_markup=channel_inline())
...
async def receive_feedback(message: Message, state: FSMContext):
    if message.text == "← На головну":
        await state.clear()
        await message.answer("Головне меню ☕", reply_markup=main_menu_kb(message.from_user.id in user_history))
        return
    await state.clear()
    await message.answer("✅ Дякуємо за відгук! 🧡", reply_markup=main_menu_kb(message.from_user.id in user_history))
...
async def choose_item(message: Message, state: FSMContext):
    ...
    item = MENU[item_key]
    sizes = item["sizes"] if isinstance(item["sizes"], list) else list(item["sizes"])
    if len(sizes) > 1:
        await state.set_state(OrderState.choosing_size)
        await message.answer(f"Обрав {item['emoji']} {item['name']}. Який розмір?", reply_markup=sizes_kb(item_key))
    else:
        await state.update_data(size=sizes[0])
        if item["has_milk"]:
...
async def confirm_order(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer("Поки чекаєш — підписуйся на наш канал 🧡", reply_markup=channel_inline())
    await state.clear()
